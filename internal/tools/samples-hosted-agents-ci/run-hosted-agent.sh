...
if [ -f azure.yaml ]; then
  ...
  # A toolbox service creates a new immutable version on every
  # deploy. Give each matrix cell its own toolbox resource so
  # container/code, toolbox combinations, retries, and
  # concurrent PRs cannot race on one shared default/version
  # list. Only this temporary azure.yaml is rewritten.
  "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/prepare-hosted-agent-ci-toolboxes.sh" \
    azure.yaml \
...
if jq -e 'has("TOOLBOX_NAME") and .TOOLBOX_NAME == ""' <<< "$DEPLOYED_ENV" >/dev/null; then
  ...
  echo "azure.yaml agent env for comparison:"
  yq '.services[] | select(.host == "azure.ai.agent") | {"environmentVariables": .environmentVariables, "env": .env}' azure.yaml || true
  exit 1
fi
...
curl -sS -H "Authorization: Bearer $TOKEN" \
     ...
     "$AGENT_URL" | tee /tmp/agent.json | jq . 2>/dev/null || cat /tmp/agent.json
echo "##[endgroup]"
...
echo "##[group]Foundry agent-version $VERSION (full JSON — error payload)"
VER_URL="$ENDPOINT/agents/$AGENT_NAME/versions/$VERSION?api-version=$API_VER"
echo "GET $VER_URL"
curl -sS -H "Authorization: Bearer $TOKEN" \
     -H "Foundry-Features: $FEATURES" \
     -H "Accept: application/json" \
     "$VER_URL" | tee /tmp/agent-version.json | jq . 2>/dev/null || cat /tmp/agent-version.json
echo "##[endgroup]"
...
if [ -n "$ERR" ]; then
  ...
  echo "##vso[task.logissue type=warning]No error message field found in agent-version response (status=$STATUS) — see full JSON above"
fi
...
cd "$WORK_DIR" || exit 1
source "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/hosted-agent-retry.sh"
if ! FIXTURE_RELATIVE=$(python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/hosted_agent_fixture.py" \
...
if [ -f "$SPEC_FILE" ]; then
  ...
  if ! python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/hosted_agent_test_spec.py" plan \
      --spec "$SPEC_FILE" \
  ...
  while : ; do
    ...
    if [ "$PROTOCOL" = "responses" ]; then
      ...
      CI_AGENT_SESSION_ID="$CI_AGENT_SESSION_ID" \
        python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/invoke_hosted_agent_responses.py" \
...
set +e
python "$REPO_ROOT/internal/tools/voicelive-e2e/voicelive_audio_smoke_test.py" \
...
azd ai agent show --no-prompt -o json 2>&1 | tee /tmp/agent-status.txt
...
AGENT_NAME="$FOUNDRY_AGENT_NAME"
cd "$WORK_DIR" 2>/dev/null || { echo "No azd working directory; skipping."; exit 0; }
fixture_relative=$(python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/hosted_agent_fixture.py" \
...
if [ -f "$spec_file" ] && [ -f "$plan_file" ]; then
  if ! python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/hosted_agent_test_spec.py" validate \
...
collect_delayed_evidence() {
  ...
  if [ "$needs_trace" = "true" ]; then
    ...
    python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/collect-hosted-agent-traces.py" \
...
while [ "$attempt" -le "$max_evidence_attempts" ]; do
  ...
  set +e
  python3 "$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/hosted_agent_test_spec.py" evaluate \
    --plan "$plan_file" \
...
cd "$WORK_DIR"
ENDPOINT="$(azd env get-value AZURE_AI_PROJECT_ENDPOINT)"
"$REPO_ROOT/internal/tools/samples-hosted-agents-ci/scripts/cleanup-hosted-agent-ci-toolboxes.sh" \
